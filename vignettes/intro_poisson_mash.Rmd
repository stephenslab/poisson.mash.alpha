...
used. For the purposes of this vignette, we just ran `pois_cov_ed` for
10 iterations, but we recommend using the default value `maxiter =
...
```{r posterior-summary}
# Rransform DE effect estimates to the scale of log2 fold change.
...
```{r plot-sharing-pattern, fig.height=5, fig.width=5}
...
pheatmap(corr,cluster_rows = FALSE,cluster_cols = FALSE,angle_col = 90,
         fontsize = 12,main = "Major prior covariance matrix (weight = 91%)")
```
