...
and here we examine the results of this analysis on the full data.

```{r load-results}
data(neutrophils_pois_mash_ruv_fit)
```

The Poisson masah result provides posterior summaries of the
condition-specific DE effects relative to the reference condition,
including: posterior means and posterior standard deviations of the
log-fold changes (LFCs); and measures of significance for the LFCs
(local false sign rates). These posterior quantities are stored in the
"result" output.  For example, this gives the posterior mean estimates
of the LFCs:
...
Here, we divided by `log(2)` to obtain LFC estimates using the base-2
logarithm, which is the convention in differential expression
analyses. (The LFCs returned by `pois_mash()` are on the natural
log-scale.)

These are the local false sign rates (lfsrs) corresponding to these
LFC estimates:
...
We can find the number of differentially expressed genes, which are
defined as genes having *lfsr* less than $\alpha$ in at least one
condition, where $\alpha$ is a threshold specified by the user
(e.g., 0.05):
...
`pois.mash.fit` component. For example, the mixture
proportions $\pi_{kl}$ for combinations of different scaling
coefficients $w_l$ and prior covariance matrices $U_k$ tell us which
sharing patterns were estimated to be most prominent in the data:
...
(91% when summed over all scalings, *i.e.*, over columns in this
plot). This is the correlation matrix obtained from this covariance
matrix (shown here for 27 of the 45 conditions):
...
This correlation matrix suggests strong sharing of DE effects among
two groups of cytokines: (1) IL-1$\alpha$, IL-1$\beta$, G-CSF and (2)
IL-12 p70, IFN-$\gamma$, IL-18, IL-15, IL-33.

Here's a few differentially expressed genes that show effect sharing
among the first group of cytokines:

```{r plot-lfc-1, fig.height=4, fig.width=2.5}
cols <- colorRampPalette(c("blue","white","red"))(99)
brks <- seq(-log2(10),log2(10),length = 100)
genes1 <- c("Prok2","Nos2","Teddm3","Lipg","Saa3","Wfdc17","Cd38","Ggt1",
            "Dpep2","Mmp19")
pdat <- t(neutrophils_pois_mash_ruv_fit$result$PosteriorMean[genes1,idx.trts])/log(2)
pheatmap(pdat,cluster_rows = FALSE,cluster_cols = FALSE,color = cols,
...
genes2 <- c("Gbp2","Gbp3","Gbp4","Gbp5","Ifi47","Ifit2","Ifit3","Socs1",
            "Tgtp2","Zbp1")
pdat <- t(neutrophils_pois_mash_ruv_fit$result$PosteriorMean[genes2, idx.trts])/log(2)
pheatmap(pdat,cluster_rows = FALSE,cluster_cols = FALSE,color = cols,
         breaks = brks,angle_col = 90,fontsize = 7,main = "")
```
