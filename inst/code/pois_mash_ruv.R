...
#' 
#' @param ulist A list of G numeric vectors each of which forming a rank-1 covariance matrix (e.g., returned by \code{pois_cov_ed})
#' 
#' @param ulist.epsilon2 A G by 1 numeric vector that adds a small positive epsilon2 to the diagonals of each rank-1 prior covariance matrix
#' to avoid tight error bars. Default is 1e-8.
#' 
#' @param normalizeU A logical scalar indicating whether to normalize the prior covariances to have maximum of 1 on diagonal. Default is \code{TRUE}.
#' 
#' @param gridmult A numeric scalar indicating factor by which adjacent grid values should differ; close to 1 for fine grid. Default is 2.
#' 
#' @param wlist An L by 1 numeric vector of scaling factors for the prior covariance matrices
#' 
...
#' 
#' @param C A Q by R matrix of contrasts for effects. Default is R by R matrix of condition-wise differences relative to the mean across all conditions
#' 
#' @param res.colnames A Q by 1 character vector giving the names of the contrasts
#'
...
pois_mash <- function(data, Ulist, ulist, ulist.epsilon2=NULL, normalizeU=TRUE, gridmult=2, wlist=NULL, 
  ...
  if(is.null(wlist)){
    ...
    w_min <- pmax(min(apply(loglambda, 1, sd)^2)/1e2, 1e-8)
