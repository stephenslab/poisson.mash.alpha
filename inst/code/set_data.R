#' @title Create a data object for poisson mash analysis
#'
#' @param Y A J by N matrix of counts with features (e.g., genes) as rows and observations (e.g., cells) as columns 
#' 
#' @param condition An N by 1 vector of factors with R levels denoting the condition status of each observation
#' 
#' @param si An N by 1 vector of size factors for each observation 
#' 
#' @param subgroup An R by 1 named vector of factors with M levels denoting the subgroup status of each of R conditions. 
#' Default to no subgroup (M=1). If not set to \code{NULL}, the names of subgroup must match the levels of condition.
#' 
#' @return A pois.mash data object for poisson mash analysis, including the following components:
