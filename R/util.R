...
# Initialize the J x 1 vector of dispersion parameter psi2, where X is J x R matrix of counts, s is R x 1 vector of sequencing depths, 
# mu is J x R matrix of means, bias is J x R matrix of bias caused by unwanted variation.
initialize_psi2 <- function(X, s, mu, bias=matrix(0, nrow(X), ncol(X))) {
  ...
  for (j in 1:J) {
    ...
    for(l in 1:length(psi2_grid))
      for(r in 1:R)
        logdens[l] <- logdens[l] + log(dpoilog(X[j,r], mu[j,r]+bias[j,r]+log(s[r]), sqrt(psi2_grid[l])))
